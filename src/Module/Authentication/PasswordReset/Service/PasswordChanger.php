...
final readonly class PasswordChanger
{
    ...
    public function changeUserPassword(array $userValues, int $userId): bool
    {
        ...
        if (isset($userValues['old_password'])
            && $this->userUpdateAuthorizationChecker->isGrantedToUpdate(['password_hash' => 'value'], $userId)
        ) {
        ...
        if (!isset($userValues['old_password'])
            ...
            && (!$this->userUpdateAuthorizationChecker->isGrantedToUpdate(['password_without_verification' => 'value'], $userId)
    ...
    {
        if ($this->userUpdateAuthorizationChecker->isGrantedToUpdate(['password_hash' => 'value'], $userId)) {
